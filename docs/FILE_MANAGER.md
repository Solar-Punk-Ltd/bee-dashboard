...
- **PARANOID**: Maximum redundancy (~4x storage)
...
File Manager enforces maximum cognitive complexity of 20 per function via ESLint. When functions exceed this limit, several refactoring patterns are applied:

**Extract Modal Rendering** - Move all modal JSX to separate component. Instead of 5 inline modals adding 5 complexity points, extract to FileBrowserModals component reducing parent complexity significantly.

**Extract Complex Conditions** - Replace multi-part conditions with named boolean variables. Instead of `isProgressModalOpen && isDestroying && currentDrive`, use `showDestroyProgressModal` constant.

**Use Callbacks** - Wrap event handlers in useCallback to prevent re-renders. Doesn't directly reduce complexity but improves performance and makes dependency tracking explicit.

**Extract Render Functions** - Move complex JSX blocks to helper functions. Reduces visual clutter and can be extracted to separate file if reused.

### Modal Pattern

All modals follow consistent structure:

**Portal Rendering** - Use React's createPortal to render modal outside component DOM hierarchy. Prevents z-index stacking issues and ensures proper overlay positioning.

**State Management** - Parent component controls modal visibility with boolean state. Modal receives handlers for cancel and confirm actions.

**Loading States** - Modals handle their own loading state during async operations. Disable buttons and show loading indicators during operations.

**Confirmation Pattern** - Destructive actions require explicit confirmation with clear warning text and secondary confirmation button.

### Progress Indicators

Three types of progress indicators are used:

**Overlay Spinners** - Semi-transparent overlay covering component with spinner and text. Used during operations like refreshing file list or destroying drive. Some overlays are clickable to expand to full progress modal.

**Progress Modals** - Full modal showing detailed progress. Include minimize button to collapse back to overlay. Used for long operations like drive destruction where user might want to monitor details.

**Transfer Notifications** - Sliding panel showing list of file transfers. Each transfer has individual progress bar, ETA, and cancel button. "Close All" button dismisses completed transfers.

### Event-Driven Architecture

File Manager uses event-driven pattern for state synchronization:

**Event Emission** - FileManagerBase library emits events when operations complete (DRIVE_CREATED, FILE_UPLOADED, FILE_TRASHED, etc.).

**Event Listeners** - Provider registers listeners for all events during initialization.

**State Updates** - Listeners call sync methods to update React state when events fire.

**Automatic Re-render** - Components using context automatically re-render when state changes.

This pattern ensures UI always reflects current Swarm state without manual refresh.

---

### Large File Lists

**Problem**: Rendering 1000+ files causes noticeable lag and poor user experience.

**Solutions**: Implement virtual scrolling using libraries like react-window to render only visible items. Alternatively, implement pagination showing 50-100 items per page with navigation controls.

### Expensive Computations

**Problem**: Filtering and sorting re-runs on every render, wasting CPU cycles.

**Solution**: Use useMemo hook to cache filtered and sorted results. Only recompute when input arrays or filter/sort parameters actually change.

### Frequent Re-Renders

**Problem**: Parent component re-renders cause all children to re-render unnecessarily.

**Solution**: Wrap child components in React.memo with custom comparison function. Component only re-renders if its specific props changed.

### Context Updates

**Problem**: Single large context causes all consumers to re-render on any state change.

**Solution**: Split into multiple contexts (FileManager, View, Search). Components only subscribe to contexts they actually use. Updates in one context don't affect components using only other contexts.

### Stamp Fetching

**Problem**: Fetching stamps from Bee on every render wastes network and slows UI.

**Solution**: Implement caching in provider with timestamp tracking. Fetch fresh stamps only if cache is older than 30 seconds. Return cached stamps for requests within cache window.
