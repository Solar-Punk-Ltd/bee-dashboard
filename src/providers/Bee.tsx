...
export function Provider({ children }: Props): ReactElement {
  ...
  const refresh = useCallback(async () => {
    ...
    const [
      ...
      beeApi.status.getHealth({ timeout: TIMEOUT }),
      beeApi.status.get({ timeout: TIMEOUT }),
      beeApi.connectivity.getNodeAddresses({ timeout: TIMEOUT }),
      beeApi.status.getNodeInfo({ timeout: TIMEOUT }),
      beeApi.connectivity.getTopology({ timeout: TIMEOUT }),
      beeApi.connectivity.getPeers({ timeout: TIMEOUT }),
      beeApi.chequebook.getAddress({ timeout: TIMEOUT }),
      beeApi.cheque.getAllLatest({ timeout: TIMEOUT }),
      beeApi.status.getChainState({ timeout: TIMEOUT }),
      beeApi.wallet.getBalance({ timeout: TIMEOUT }),
      beeApi.chequebook.getBalance({ timeout: TIMEOUT }),
      beeApi.stake.get({ timeout: TIMEOUT }),
      beeApi.balance.getAll({ timeout: TIMEOUT }),
      beeApi.settlement.getAll(),
    ])
