...
function buildDatesGroup(createdTs?: number, modifiedTs?: number, expires?: string): FilePropertyGroup {
  return {
    ...
    properties: [
      ...
      { key: 'accessed', label: 'Last Accessed', value: dash }, // not tracked
    ...
    properties: [
      {
        key: 'owner',
        label: 'Owner',
        value: owner ? truncateMiddle(String(owner), 12, 8) : dash,
      ...
      { key: 'grantees', label: 'Grantees', value: granteeCount != null ? `${granteeCount}` : dash },
      {
        key: 'actpub',
        label: 'ACT Publisher',
        value: actPublisher ? truncateMiddle(String(actPublisher), 12, 8) : dash,
        raw: actPublisher ? String(actPublisher) : undefined,
      },
      {
        key: 'historyRef',
        label: 'ACT History',
        value: historyRef ? truncateMiddle(String(historyRef)) : dash,
    ...
    properties: [
      {
        key: 'batch',
        label: 'Batch ID',
        value: batchId ? truncateMiddle(String(batchId), 12, 10) : dash,
        raw: batchId ? String(batchId) : undefined,
      },
      { key: 'drive', label: 'Drive', value: driveLabel ?? dash },
      { key: 'redundancy', label: 'Redundancy', value: redundancyLevel != null ? String(redundancyLevel) : dash },
    ],
  }
}
