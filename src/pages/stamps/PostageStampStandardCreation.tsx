...
import { DEFAULT_BLOCK_TIME } from '../../constants'
...
export function PostageStampStandardCreation({ onFinished }: Props): ReactElement {
  ...
  const [amountInput, setAmountInput] = useState<bigint>(
    Utils.getAmountForDuration(Duration.fromDays(30), 26500, DEFAULT_BLOCK_TIME),
  )
  ...
  function sliderValueChange(_: unknown, newValue: number | number[]) {
    ...
    const amountValue = Utils.getAmountForDuration(Duration.fromDays(newValue), 26500, DEFAULT_BLOCK_TIME)
  ...
  function getTtl(amount: bigint): string {
    ...
    return `${secondsToTimeString(
      Utils.getStampDuration(amount, pricePerBlock, DEFAULT_BLOCK_TIME).toSeconds(),
